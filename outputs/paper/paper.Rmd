...
Unless you've been living under a rock, the question in everyone minds is who will the 2020 US presidential election. Polls are consistently being updated, in an attempt to reflect the newest voter sentiment as new controversies break daily. In the past few months, candidates have been on the campaign trail holding rallies and attending debates, all in the hopes of swaying various demographic groups in the US in order to gain their votes. By choosing 5 explanatory demographic variables closely associated with voting and political affiliation (age, gender, state, race, and education), we develop a multi-level regression model with post-stratification in order to predict the outcome of the popular vote. 

In our analysis, we modeled the relationship between our selected demographic variables and a person's likelihood to vote for either Trump or Biden. We analyzed the significance of our model, and the importance of using multilevel regression with post-stratification because the training data is not proportional to the American population. We found that we were able to make predictions with approximately 63% accuracy and the strongest indicators of 2020 voting are education and gender. Furthermore, only certain values within our variables are strong indicators of a person's forecasted vote. For examples, 90% of the Vermont population is projected to vote for Joe Biden, and approximately 90% of Black or African Americans are likely to vote for Biden. However, not many variables provided strong support for Donald Trump, noticeably 70% of the state of Arizona is likely to vote for Trump. However, individually, many of our variables are close to 50% and do not provide a strong indication on an individuals voting intention. 

This paper discusses the 2 datasets we used, how they were collected and key highlights of these datasets, followed by visualizations of the data. Next we explained the construction of our model and the positives and negatives of extrapolating information from a smaller voter survey to the American population using a post-stratification dataset. Finally, we present our results, namely our cross-validation accuracy, the estimated coefficient intervales for our model, and our projected predictions on the post-stratification dataset. 
...
```{r, echo=FALSE}
# Instructions on how to access this data file are posted in the README in our Github repository
# Load Nationscape dataset for Wave 49
raw_data <- read_dta("../../inputs/data/ns20200625/ns20200625.dta")
raw_data <- labelled::to_factor(raw_data)
# Generate summary frequencies for variable vote_2020
...
```{r, echo=FALSE}
# Generate summary frequencies for variable age
...
```{r, echo=FALSE}
# Generate summary frequencies for variable gender
...
```{r, echo=FALSE}
# Generate summary frequencies for variable race_ethnicity
...
```{r, echo=FALSE}
# Generate summary frequencies for variable education
...
tibble_race$race <- factor(tibble_race$race, 
                                      levels = c("White", "Black, or African American", 
                                                 ...
                                                 "Some other race"))
# Distribution of race as a bar graph in percentages
...
# Summary table with counts for education
tibble_education <- data.frame(table(reduced_data$education)) %>%
  rename(
    education = Var1,
    count = Freq
  )
# Distribution of education as a bar graph in percentages
...
# Summary table with counts for state
tibble_state <- data.frame(table(reduced_data$state)) %>%
  rename(
    state = Var1,
    count = Freq
  )
# Distribution of state as a bar graph in percentages
...
# Summary table with counts for vote
tibble_vote <- data.frame(table(reduced_data$vote_2020)) %>%
  rename(
    vote = Var1,
    count = Freq
  )
# Distribution of vote as a bar graph in percentages
...
# Plot distribution of age as a bar graph in percentages
...
# Plot distribution of race as a bar graph in percentages
...
# Calculate percentages for each education group
perc_education <- cleaned_data %>% count(education) %>% mutate(perc = n/nrow(cleaned_data))
# Plot distribution of education as a bar graph in percentages
...
# Load cleaned processed data (generated by script build-model.R)
reduced_data <- readRDS("../../inputs/training_data.Rda")
total_rows <- nrow(reduced_data)
# Get select columns of interest
...
set.seed(50)
# Complete cross-validation by reproducing testing set
# Shuffle indices in the same way as build-model.R
...
# Separate training and testing datasets using calculated boundary
...
# Graph cross-validation results, TRUE being a correct prediction
...
```{r}
# Find coefficient estimates and intervals for 95% confidence level
coefficients_text <- summary(model_education)
```

```{r, echo=FALSE}
# Hand input summary coefficients
...
# Saved by running mrp.R
cell_counts <- readRDS("../../inputs/cell_counts.RDS")
# Saved by running build-model.R
...
# Plot predicted votes by gender
gender_res %>% 
...
# Plot predicted votes by education
education_res %>% 
