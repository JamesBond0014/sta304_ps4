...
# model <- readRDS("model/3chain_2k-iter_no-edu.rds")
```
...
```
```{r}
#run this for faster but similar results
model <- nnet::multinom(vote_2020 ~ state + race_ethnicity + gender +age_group,
                        data = reduced_data)
```
```{r}
testing <- data[boundary:total_rows,]
...
probability <- predict(model, type = "probs", newdata = testing)
probability <- if_else(probability>0.5,1,0)
...
```{r}
cleaned_post$probability <- predict(model, type = "probs", newdata = cleaned_post)
cleaned_post$probability
```
